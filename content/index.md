Howdy Aggies!

This is the ENGR4BBs project, where engineering students can find study tips, class notes, and other tools to succeed with the Entry-to-a-major process and as an engineering student. This notebook is maintained by students for student at the College of Engineering, Texas A&M University.

This notebook is useful for students starting engineering, ENGR 120, PHYS/ENRG 216 and 217. 

To get started, take a look at [[Motivation and Mindset for Engineering]]. To get comfortable with using this notebook, take a look at [[User Guide]].

Maintaining this notebook is a collective effort. Take a look at [[Contribute]] for ways you can contribute to the project.

This is a work in progress. If you have any concerns about the content of this site, please [Contact me.](Contact)

## Road map

| **Task**                                                                            | **Timeline** | **Assignee** |
| ----------------------------------------------------------------------------------- | ------------ | ------------ |
| Complete the first entry for ENGR/PHYS 216 Lab Section                              | Summer 2026  |              |
| Complete ENGR 102                                                                   | Summer 2026  |              |
| Host to web using Quartz by Jacky Zhao                                              | Summer 2026  |              |
| Seek recommendation from TAO and Dr Ritchey                                         | Fall 2026    |              |
| Add CSCE 210                                                                        | Undecided    |              |
| Expand to PHYS 206 and PHYS 207 to cover the foundational knowledge with more depth |              |              |


